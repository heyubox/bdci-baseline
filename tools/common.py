...
def get_recent_checkpoint(output_dir):
    dirs = os.listdir(output_dir)
    valid_dirs = list(filter(lambda x: 'checkpoint' in x, dirs))
    valid_dirs.sort(key=lambda fn: os.path.getmtime(output_dir + "/" + fn))  # 按时间排序
    new_dir = os.path.join(output_dir, valid_dirs[-1])
    return new_dir
