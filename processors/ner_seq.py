...
class BankProcessor(DataProcessor):
    ...
    def _read_csv(self, data_file):
        ...
        if 'test' not in data_file:
            ...
            data['BIO_anno'] = data['BIO_anno'].apply(lambda x: x.split(' '))
            data['training_data'] = data.apply(lambda row: (list(row['text']), row['BIO_anno']), axis=1)
            for words, labels in data['training_data']:
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        # return self._create_examples(self._read_csv(os.path.join(data_dir, "train_data.csv")), "train")
        return self._create_examples(self._read_csv(os.path.join(data_dir, "train_data_public.csv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        # 从train data抽取部分
        return self._create_examples(self._read_csv(os.path.join(data_dir, "eval_data.csv")), "dev")
    ...
    def get_labels(self):
        """See base class."""
        return ["X", 'O', 'B-BANK', 'I-BANK', 'B-PRODUCT', 'I-PRODUCT',
